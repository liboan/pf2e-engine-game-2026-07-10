...
**Smallest packet/decision.** `authority-and-semantic-closure`: immutable multi-record fact sets, typed transitive edges, cycle/digest rules, and one closeable subject for every executable leaf. At this review baseline the executable-leaf boundary was open; [decision 0002](../../decisions/0002-foundational-rules-contracts.md) now makes capabilities support claims and terminates executable closure in inventoried core-procedure or fixed-module semantic nodes. Compiler output, registry joins, package loading, and source-reviewed evidence must reconcile that graph. This is an architecture choice, not a PF2e interpretation.
...
**Smallest packet/decision.** `effect-turn-and-health-lifecycle`: v1 defines the damage-result handoff, actor category, zero-HP result, defeat/destruction/dying and initiative transition, reaction boundary, save/retry, and public delta. Later versions add the phase DAG and owner, `PhaseOrderDecision`, condition contributions/effective projection, effect equivalence/immunity scope, duration anchors, and simultaneous groups. At this review baseline, actor categories and significant-NPC policy were open; [decision 0002](../../decisions/0002-foundational-rules-contracts.md) now selects PC-category dying/wounded, ordinary-creature defeat at 0 HP, and unsupported significant-NPC exceptions. Detailed health schemas and discretionary simultaneous/duration interpretations still require their named packets and focused oracles.
...
- Larger footprints, reach, terrain costs, areas, elevation, flight, climb, swim, and other movement modes are explicitly outside the first gate. That is not a defect. At this review baseline diagonal movement was open; [decision 0002](../../decisions/0002-foundational-rules-contracts.md) now includes it. The spatial/accounting contract must implement S05b's turn-scoped diagonal ledger, persist it through save/resume, and reset it only at the approved boundary.
