# Content W3 working model

**W2 and W3 are accepted; execution is closed.** The existing heartbeat is paused. Final evidence and authorized W2/W3 publication are recorded in [ACTIVE](../work-log/ACTIVE.md) and [W3 acceptance](../work-log/archive/content-w3-accepted.md). The retained workflow below governs concrete repairs and future separately authorized work; it does not launch another wave. Preserve accepted ancestry, exact remote/local delivery verification and clean checkout. No force-push, main merge or unrelated overwrite; no new PR requirement.

This working model supersedes the historical Terra-only, native-subagent-only and completed-generalization-only execution rules. The user authorized Content W3 now, reusing the existing six tasks, roles/models and worktrees, with broader coherent families and corrected routing. W1/W2 are accepted; W3 is a new assignment, not replacement tasks. No further planning approval is needed. Historical evidence is preserved in [the previous recovery snapshot](../work-log/archive/active-before-content-w1-2026-09-20.md) and [previous workflow](../work-log/archive/working-model-before-content-w1-2026-09-20.md); their execution instructions are not current.
...
Start from existing source packets and accepted tests. Each owner briefly assesses a few candidates for useful shared mechanics, legal access, material differences, likely duplication and excluded expensive prerequisites. W3 must materially exceed W1-sized slices and W2 breadth: select multiple related class feats and/or a shared mechanism enabling multiple actual content/class adopters. A solitary option or relabeling existing setups is insufficient. Prefer caster feat/preparation/spellshape breadth, martial strike riders and finite bomb riders, with shared behavior adopted by old/new callers. Keep a brief source-grounded assessment inside the assignment, then implement the whole bounded family. Size by shared mechanics, uncertainty and playable usefulness, never an arbitrary count. Keep the candidate assessment concise with the owner; send it with actionable review material, not a separate coordinator update. If a lane has no suitable family or a scope/contract decision is needed, send the smallest actionable blocker to the appropriate peer. No exhaustive catalog or approval per feat.
