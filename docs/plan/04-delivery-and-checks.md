...
|---|---|---|
| Straightforward scoped implementation, focused tests, small debugging, documentation integration | `gpt-5.6-terra`, `high` | User restored this routing on 2026-09-18, superseding the attempted Luna rollout. Retain one owner through ordinary repairs. |
...
The completed generalization and simplification phase was coordinated by Astra/high task `01a0b6ce-40ce-7f51-8042-b9f2b8956421`. It used directly managed Terra/high owners and Sol/high reviewers. The user subsequently authorized a fresh Sol/high delivery coordinator in a new managed worktree; do not resume the two older coordinators, their workers, or their heartbeats. Native Terra adoption must be verified from actual metadata. Preserve unrelated configuration and the custom catalog; no Luna probes or app restart are required.

Name one coherent refactoring group with demonstrated existing adopters for each assignment. Prefer one retained Terra/high owner across runtime, persistence, terminal and tests, with independent Sol/high review of the complete runnable group. Keep frequent focused executable checkpoints internal: passing one migrated feature does not trigger acceptance, accounting closure or redispatch. Require actual shared adoption and deletion of replaced paths, not merely relocation. Distinguish reproduced correctness fixes from behavior-preserving extraction. Split only for material rules uncertainty, foundational/shared-contract risk, ownership contention or failure isolation.

One owner per outcome does not limit the whole project to one implementer. Check for one already-authorized second outcome with useful executable work independent of unsettled prerequisites and shared edits; delegate a bounded dependency/ownership check if needed. Separate filenames alone do not establish independence. If no safe lane exists, record its concrete dependency briefly and continue current work. Do not manufacture planning work, divide layers or broaden scope to occupy capacity. Launch a dependent helper only after its required API exists and a public probe passes.

Worker reports remain event-driven: a decision blocker, shared-contract change, first executable checkpoint or final evidence. The five-minute diagnosis is a one-time startup safeguard when useful execution evidence is absent: a localized failing check or passing partial path is useful. It is not a recurring timer and does not end the assignment. Keep at most one outstanding status request per worker; another requires new evidence, a missed agreed checkpoint or a dependency decision. An app progress snapshot can omit actual child activity; missing messages/tool markers do not establish a startup failure or lack of progress. Obtain a direct worker checkpoint before interrupting or changing ownership, and retain the worker when it is making progress. Split or reassign only for demonstrated scope expansion, unresolved dependency or repeated lack of progress, not elapsed time alone. Preserve productive workers and completed research when changing procedure. A final handoff is usually 250–400 words with commands/results/evidence; avoid acknowledgements and repeated status. Use absolute edit paths and permit routine adjacent refactoring within scope.

Call native collaboration tools directly, outside `functions.exec`, addressed to the agent name/path. `collaboration.send_message` queues information but does not start a new turn for an idle/completed recipient; use it for reports and information that can wait. Use `collaboration.followup_task` for an already-authorized review activation, within-scope repair request or dependency response requiring the recipient to act, including when the peer may have ended its turn. Peers can use it: it starts an idle non-root recipient or delivers the follow-up to a running recipient without interruption. Successful submission is not proof of handling; use the next substantive response/activity or an explicit blocker as evidence. Native tools are separate from `ALL_TOOLS`; searching that list cannot establish absence. Do not use app cross-task messaging for subagents or routinely send test pings/acknowledgements. If a native tool is actually absent or fails, record the exact limitation once and use compact commentary/final reports, with root relaying necessary dependencies. Within-scope synchronization and ordinary repairs retain their assignment/accounting boundary. A user-requested one-time receipt check is an exception, not a recurring broadcast.
...
Reuse the assignment's existing source packet and acceptance examples; give the coherent bounded public outcome to one Terra/high owner. Related grants can reuse the existing encounter and internal passing checkpoints, as Knowledge/Known Weaknesses already did and Anthem/Lingering could do in a future composition group. This reduces handoffs and repeated acceptance/setup reasoning while preserving frequent incremental tests. For level-2 delivery, the default assignment and review boundary is a whole selected class progression package or a substantial multi-class mechanic family—not an individual feat or prerequisite. A smaller shared-API slice is justified only when it unlocks at least two named real adopters and remains with its integration owner through those adopters. Do not reopen completed assignments. Do not add work packets, acceptance tables, exhaustive clause inventories, mandatory planning agents or approval steps for this procedure. Workers translate source expectations into tests and continue through authorized related successors without routine coordinator approval. Root receives outcomes, blockers, unmet examples, concise check results and links; correctness detail stays with workers. Preserve current acceptance gates, independent lanes and pending user adjudications.
...
Use Sol selectively at coherent runnable completion boundaries; this is not a review for every implementation assignment. At the next already-warranted boundary, root may preauthorize the named retained reviewer, bounded review scope and coherent runnable readiness condition. Once that condition passes, the implementer activates the review directly with native `collaboration.followup_task`, without waiting for a coordinator wake, provided its accurate accounting boundary was captured before activation. New or expanded scope still belongs to root. Reviewers execute the Python engine, check sources and whether assertions establish the claimed rules, and try a few relevant independent interactions. Distinguish missed specified requirements, implementation defects, reviewer-test errors and optional coverage; optional coverage alone does not reopen acceptance. Owner and reviewer use `followup_task` directly for ordinary repair requests and actionable dependency responses, then report the resolved outcome; these are not new accounting assignments. Escalate rules, scope, ownership or shared-contract decisions. Retain both through repairs; preserve complete-play, save, integration and performance gates.

An available Sol reviewer may consult early on a named source/test uncertainty likely to cause substantial rework, before dependent implementation and independently of ongoing edits. Return a concise source-backed conclusion, expected behavior and any remaining decision in the existing handoff. Reuse completed research. Do not make early consultation mandatory or commission an investigation merely because an agent is idle; pending user adjudications remain pending.
...
Content delivery has resumed for the approved representative-build milestone: finish the remaining selected level-1 Bard and Monk, then their approved level-2 progression. Preserve the completed generalization result, accepted play, and unsupported boundaries; specialized printed rules may remain specialized. No exhaustive option inventory, universal rules language, speculative framework, unrelated content expansion, or wholesale rewrite is authorized.
...
- Before implementing affected behavior, express the few easily confused source-grounded expectations directly in existing tests, independently of observed output. Distinguish attack from damage, caster from target expiry, immunity from actor-side benefits, and restoration from actual resource consumption where relevant. Briefly cite the source or explain a non-obvious distinction. Resolve source/test disagreements; do not change expectations merely to match output. Escalate genuine rules uncertainty and keep ordinary corrections with the owner.
- Focused unit tests check shared calculations and state transitions.
- Each encounter must demonstrate its claimed interaction. A pre-injured fixture does not prove healthy-start rescue; a cantrip does not prove consumption of a restored slot. Check legal selected-build grants and proficiency metadata when relevant. Merely placing content on the map is not evidence that its rules were exercised.
...
[Current work](../work-log/ACTIVE.md) is the sole compact recovery surface for current owners, outcomes, dependencies, last evidence and next actions. Replace stale state at meaningful checkpoints instead of appending narratives; keep completed detail linked. STATUS and README point there rather than duplicating the queue. Preserve productive workers and completed packages when changing guidance; do not create a fresh coordinator or backfill completed work.
...
python3 tools/log_agent_usage.py dispatch --log docs/work-log/agent-runs.json \
  ...
  --task 'Bounded assignment' --model gpt-5.6-terra --effort high
...
Workers proactively report blockers, meaningful executable checkpoints and completion, and continue independently after the parent turn ends. Use the five-minute execution heartbeat attached to this task for coordinator check-ins. This replaces keeping a parent turn open through milestone completion. The app implements the interval as an inactivity/cooldown period after the latest relevant thread update/run, not fixed wall-clock ticks; active or waiting target turns are deferred and the scheduler excludes an already-running automation. Do not start another coordinator or interrupt productive workers to force a tick.

On each scheduled wake, read ACTIVE and the relevant assignment, inspect known active workers and newly available reports once, resolve actionable blockers, accept completed evidence and dispatch the next authorized work when dependencies are ready. Routine findings and repairs stay directly between owner and reviewer. If runnable work is assigned and no immediate coordinator decision remains, end the turn; if no action is needed, end quietly. No wait loop, routine status request or unchanged-record rewrite. Batch bookkeeping at meaningful changes and notify only for meaningful progress, completion, failure or a required decision. **User-directed exception to ending quickly or quietly:** whenever a worker check discovers that a worker is idle waiting for clarification, surface that blocker directly before ending the turn. State which work is held, the exact unresolved decision and the recommendation; use the structured question tool when an answer is needed. A question being previously pending is not a reason to hide the idle dependency. Continue independent work and avoid wait loops or routine status polling.

The five-minute execution heartbeat `pf2e-content-delivery-execution` belongs only to the fresh delivery coordinator and advances the resumed representative-build milestone. The older `pf2e-generalization-execution` and `pf2e-content-expansion-execution` heartbeats remain paused; leave their coordinators, workers, and the separate audit automation unchanged. Pause the new heartbeat when the milestone completes or the user pauses it. Configuration is not runtime-cycle proof: record the first actual same-task scheduled wake and retained-worker follow-up once observed; do not manufacture scheduler probes or wait loops.
...
Clarify P1 uncertainty before a materially dependent design or product commitment. If a worker becomes idle on that unanswered question, bring it directly to the user when discovered; the scheduled coordinator must not silently leave it buried in the log. Routine reversible choices use agent judgment and a brief note. Do not manufacture approval gates for ordinary implementation or content already inside the user's authorized scope.
...
Within the usual compact handoff, link the tests proving the assignment's existing acceptance examples and name unmet examples explicitly. Report accepted behavior, concise checks and measurements, elapsed time, usage/cost, ownership changes, review repairs, limits and next action. No separate acceptance deliverable is required. Do not count unsupported boundaries as delivered or silently drop requirements. Git commits and changes in other worktrees require actual execution.
