...
The user narrowed S3i to representative builds from all sixteen Player Core 1/2 classes: complete level 1 first, then level 2, with limited common feats/spells/items and explicitly granted rune test equipment. Every selected option must work correctly; unused subclasses and nested choices may remain deferred. Use one small shared domain menu and finite alchemical/casting selections. The selected builds' required class features bring their shared mechanics into this milestone. After both levels work, review opportunities to simplify repeated implementation. The [extension plan](06-class-and-content-expansion.md) owns this scope and its acceptance checkpoints.
