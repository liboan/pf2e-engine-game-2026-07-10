...
| **S3i — Interaction encounter expansion** | Add 8 distinct S2 encounters and 8 distinct S3 encounters before adding later-stage mechanics. | Exercise supported mechanics together in complete fights, broaden useful low-level content within those mechanics, and retain meaningful save/resource/health/positioning milestones. Use modular bounded tests and periodic full play. See the [interaction work item](05-interaction-encounters.md). |
| **S4 — Common tactical variety** | New encounters add maneuvers, terrain, and persistent consequences. | Build on existing flanking/off-guard, prone/Stand and creature cover. Prioritize Trip/Grapple/Shove/Escape, Demoralize, shields, terrain cover, difficult terrain, persistent damage, and useful action restrictions. Add areas/Sustain or a larger footprint/reach example as a small separate increment. Implement only the selected subset at a time. |
| **S5 — Broader everyday play** | More content mostly uses existing rules, with deliberate additions that unlock substantial variety. | Additional casting sources, focus/innate uses, ordinary consumables/runes, weakness/resistance/immunity interactions, Ready/Delay, restricted extra actions, recovery between fights, senses/detection, simple hazards, and common auras/afflictions when justified. Dependencies can pull a small part earlier. |
| **S6 — Selective specialist work** | Individually chosen specialist encounters or options. | Flight/vertical space, swimming, summons, companions, mounts, transformations, complex hazards, counteracting, and unusual timing only when their value warrants the cost. They never become a blanket gate for ordinary encounters. |
...
- **Warpriest:** use one reviewed prepared divine casting model, including its separate Heal-only font slots. Implement Divine Lance, Void Warp, Guidance, Stabilize, and Heal at the selected rank. Read Aura remains a visible exploration option outside the encounter runner.
...
### S3i working brief — completed interstitial work
