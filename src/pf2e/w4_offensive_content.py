    ...
    *, fighter: CreatureDefinition, ranger: CreatureDefinition, rogue: CreatureDefinition,
    ...
    exacting = replace(
        ...
        sheet_notes=(*fighter.sheet_notes,
                     ...
                     "Source: https://2e.aonprd.com/Feats.aspx?ID=4770"),
    )
    double_slice = replace(
        ...
        sheet_notes=(*fighter.sheet_notes,
                     ...
                     "Source: https://2e.aonprd.com/Feats.aspx?ID=4769"),
    )
    ranger_twin = replace(
        ...
        abilities=tuple(item for item in ranger.abilities if item != "hunted_shot") + ("twin_takedown",),
        feats=tuple(item for item in ranger.feats if item != "Hunted Shot") + ("Twin Takedown",),
        sheet_notes=(*ranger.sheet_notes,
                     ...
                     "Source: https://2e.aonprd.com/Feats.aspx?ID=4864"),
    )
    rogue_twin = replace(
        rogue, definition_id="w4_rogue_twin_feint_level_1",
        name="Level 1 Rogue (Twin Feint)",
        attacks=rogue.attacks + (
            replace(_shortsword(modifier=7, damage_modifier=4), attack_id="dagger", item_id="dagger", name="Dagger", damage_dice=(4,), traits=frozenset({"attack", "melee", "agile", "finesse", "thrown", "weapon"}), damage_type="piercing"),
        ),
        held_items=("shortsword", "dagger"),
        abilities=tuple(item for item in rogue.abilities if item != "nimble_dodge") + ("twin_feint",),
        feats=tuple(item for item in rogue.feats if item != "Nimble Dodge") + ("Twin Feint",),
        sheet_notes=(*rogue.sheet_notes,
                     "W4 selected class feat: Twin Feint. The finite fixture holds a shortsword and dagger and exposes two different wielded melee Strikes; the second attack gets the feat's automatic off-guard benefit.",
                     "Source: https://2e.aonprd.com/Feats.aspx?ID=4921"),
    )
