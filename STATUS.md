...
The authorized generalization and simplification phase is complete. It accepted three coherent groups: shared spell-save provenance across ten paths; finite preparation routing and validation across Wizard, Druid, Witch, and fixed preparations; and existing finite `BombFacts` adoption for two admitted bombs. Relevant defects were repaired and independently reviewed. Final canonical verification, compile/diff cleanliness, and unchanged saved-continuation evidence are recorded in [ACTIVE](docs/work-log/ACTIVE.md), which remains authoritative for live counts, accepted level-1 coverage, partial Bard and Monk work, and level-2 acceptance state.
