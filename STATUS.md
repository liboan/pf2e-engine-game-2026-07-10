...
## Active work — S3i class and content expansion

The target is one selected representative build for each of the 16 Player Core and Player Core 2 classes. Finish level 1, then level 2, then have Astra review demonstrated implementations for useful generalization. Each representative needs its mandatory grants and selected rules behavior, an actual encounter, and saved continuation evidence. Every printed subclass and nested option is not required; unselected source inventories are reference material rather than delivery backlog. Feat, spell, school, domain, and Alchemy menus remain deliberately small. Explicitly granted, labeled above-level fundamental-rune test gear does not change character level. See the [scope and acceptance plan](docs/plan/06-class-and-content-expansion.md).
...
Finish and accept the selected level-1 representative for each class with complete required behavior, curated optional content, complete encounters, saved continuations, and representative terminal use. Then implement and accept actual level-2 progression for those representatives. After both levels pass, have Astra review demonstrated implementations for useful generalization and apply justified changes. These are acceptance steps within S3i, not later milestones.
